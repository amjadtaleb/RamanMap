...
class RamanMap:
    ...
    def __init__(self, input_file, resolution=1600):
        ...
        self.file = '/mnt/E28A226A8A223B83/RamanMaps/' + file_name
...
# =============================================================================
    ...
    def getRaman(self):
        ...
        sneak = pd.read_csv(f, delimiter='\t', header=None,
                            nrows=1, index_col=0)
        ...
        def getStep(array):
            ...
            '''
            y, ix = np.unique(np.subtract(
                array[1:], array[:-1]), return_counts=True)
            return round(y[ix.argmax()], 3)

        if sneak.shape[1] == 0:
            print(
                '*_*_*_*_* This file useses "," as delimiter and as decimal separator *_*_*_*_*')
            delimiter = ','
            sneak = pd.read_csv(f, delimiter=delimiter,
                                header=None, nrows=1, index_col=[0, 1])
            dim = int(np.sqrt(sneak.shape[1]))
            ncols = int(dim**2)
            index = pd.read_csv(f, delimiter=delimiter, header=None, nrows=resolution, usecols=[
                                0, 1], dtype={0: str, 1: str})
            df = pd.read_csv(f, delimiter=delimiter, header=None, nrows=resolution, usecols=range(
                2, ncols+2))  # +2 since two are index
            ...
            if (sneak.shape[1] < 5) and (sneak.shape[1] > 0):
                ...
                nrows = int(dim**2)
                print('File containes %g lines, loading the first %g into DataFrame' % (
                    count, nrows))
                df = pd.read_csv(f, delimiter=delimiter, header=None, decimal=decimal, nrows=nrows, names=[
                                 'wave', 'Int'], index_col=False)
                ...
                # rows are single measurements, cols are wavelengths
                df = pd.DataFrame(data=mat, columns=waves)
...
#                    df.columns = waves
                ...
                dim = int(np.sqrt(sneak.shape[1]))
                ncols = int(dim**2)
                df = pd.read_csv(f, delimiter=delimiter, header=None, index_col=0, usecols=range(
                    ncols+1), decimal=decimal, nrows=resolution).T
...
#        self.selected_waves_total = self.mean.iloc[peak_indices].index.values
        df_peaks = pd.DataFrame(peaks, index=peak_indices)[['prominences', 'peak_heights', 'widths']].sort_values(
            by='prominences', ascending=False).head(self.n_peaks)
...
#        self.fwhm = fwhm
        self.groups = [self.waves[i[0]-i[1]:i[0]+i[1]]
                       for i in zip(df_peaks.index.values.round().astype('int'), 2*fwhm.values)]
...
# =============================================================================
    ...
    def sliceMap(self, i):
        ...
        y_d_mn, y_d_md, y_d_mx = [sub_df.loc[i]
                                  for i in self.min_med_max(g_max)]
        y_i_mn, y_i_md, y_i_mx = [sub_df.loc[i]
                                  for i in self.min_med_max(intensities)]
...
#            if i < training_features:
        ...
        if x == True:  # scale data before or after pca, requires better separation of pcas
            ...
            self.training_data = np.hstack(
                [np.column_stack((i_s[:, n], x_s[:, n])) for n in range(self.n_features)])
        else:
            #            self.training_data = np.array(ints).T
            self.training_data = np.hstack([np.column_stack(
                (np.array(ints).T[:, n], np.array(g_xs).T[:, n])) for n in range(self.n_features)])
...
# =============================================================================
    def predictorOptimizer(self, n_features):
        ...
        '''
        print('prepare_training_data with', n_features,  'featuers')
